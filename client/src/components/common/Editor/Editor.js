		...
		return (
			<div className={styles.container}>
				<nav className={styles['nav']} onClick={this.toggleEditorState}>
					<button className={`${styles['button-base']} ${_isPreview ? '' : styles.selected}`}>编写</button>
					<button data-state="prev" className={`${styles['button-base']} ${_isPreview ? styles.selected : ''}` } >
						预览
					</button>
				</nav>
